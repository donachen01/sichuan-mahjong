...
- 结算页现在真正渲染当前桌布皮肤的纹理与色调，不再把已加载的纹理遮在面板底色后方。
- 点炮胡的胡牌张作为独立牌显示，悬浮箭头直接锚定在该牌上方并保留点炮座位；牌数较多时整行统一等比适配，不再把最后一张挤出可见区。
- 分数明细正文和得分数字放大，增加手机可见、可拖动的粗滚动条；iOS 触摸输入不再被全屏结算层提前吞掉。
- 发布时必须在 iOS 成品内检查 `SichuanMahjong.Godot.framework`，避免只有 PCK 而缺少 NativeAOT 运行时的启动闪退包。

## 2.6.60 - 2026-09-13

- AI 提示接入真人响应窗口，使用正式 C# AI 比较胡、杠、碰与过，并显示对应牌张、可选动作和核心原因。
- 本家自摸、暗杠、补杠与继续出牌进入同一操作建议界面；AI 核心无结果时明确提示不可用，不由 UI 自行猜测。
- 提示框展开与收起状态均保留动作摘要，原有推荐出牌、触控操作和规则执行链路不变。

## 2.6.59 - 2026-09-13

- iOS 改用 Godot Mobile 渲染器；移动端操作期 60 FPS、静止期 30 FPS并启用低处理器模式，静止且无 AI 后台任务时主场景帧回调完全休眠。
- 移动端关闭动态方向光阴影、绒布各向异性和高成本牌体材质分支；最新弃牌标记改为静态，避免空闲牌桌持续动画。
- 相同 3D 牌状态不再重复配置材质；AI 后台队列与看门狗改为有任务才轮询，牌局快照不再触发整棵 UI 字体树刷新。
- 保持点炮与抢杠胡的公开胡牌张及立体来源箭头始终可见，不受明牌开关影响。

## 2.6.58 - 2026-09-12

- 移动端锁定 60 FPS 并关闭 SSAO，降低 iPhone ProMotion 高刷新和环境遮蔽的持续 GPU 负载。
- 静态圆形按钮由每帧重绘改为按钮状态事件驱动，演出调度器在队列空闲时停止 `_process`。
- AI 后台结果以 30 Hz 轮询，避免随 60/120 Hz 显示帧频繁跨边界调用，不改变麻将规则与出牌动画时长。

## 2.6.57 - 2026-09-12

- 结算页补齐连续标题背景，以边角如意云纹替代重复圆环角饰，并为点炮胡牌张恢复立体来源箭头。
- 修正流局错误轮庄；首胡坐庄、一炮多响由点炮者坐庄。
- 点杠按点杠者 2 分、其他未胡玩家各 1 分即时结算；暗杠各 2 分、补杠各 1 分，呼叫转移使用真实杠钱合计。
- 核验胡牌番型继续使用暗手与副露共同判定，所有四张同牌（含杠）计根。

## 2.6.56 - 2026-09-11

- 按目标图重做全屏结算页，新增原创蜀风云纹山水装饰层、金边米白卡片、大字标题与玉色下一局按钮。
- 中部收敛为两行：本家/缺门/碰/点炮来源标签同行，放大的手牌、副露和胡牌张同行。
- 结算底板真实加载当前桌布纹理；换肤仅改变纹理与主色，不改变结算布局。

## 2.6.55 - 2026-09-11

- 将四川话男声替换为“温暖阿虎 2.0”，本家和对家继续使用男声，上家和下家继续使用女声。
- 四川话男声改为同组连续生成、按静音切分，避免短牌名单独生成时出现明显音色漂移。
- 增加本家和上家在四川话模式下实际音频资源路径的运行时验证，防止配置为男声但错误加载女声包。

## 2.6.54 - 2026-09-11

- 新增普通话和四川话两种语言、男女共四套牌局语音；本家/对家使用男声，上家/下家使用女声，并可在左上工具栏即时切换。
- 27 张筒、条、万出牌音频严格只播报牌名，不附加角色口头语；碰、杠和过保留短促角色化表达。
- 将点炮胡、自摸、抢杠胡、杠上花、杠上炮拆分为独立音频键和短句，避免用同一“胡”覆盖不同结果。

## 2.6.53 - 2026-09-11

- 缩小左上工具栏并让展开状态与上家铭牌动态避让；本家铭牌下移，对家铭牌向左调整。
- 放大桌布皮肤选择器、卡片和文字，并按工具栏真实占用区域将选择器布局在不重叠的可用区。
- 修正 AI 提示首帧只有后台请求空壳的问题，恢复提示框居中、iOS 触摸拖动并放大面板和文字。

## 2.6.52 - 2026-09-11

- 将上家铭牌下移至放大后的左上工具按钮下方，消除收起状态下按钮与舒燕铭牌的遮挡。
- 同步发布 Android Release APK 与 iOS 个人开发者真机版本。

## 2.6.51 - 2026-09-11

- 将左上工具抽屉按钮与文字扩大至上一版约两倍，改用双列向下展开布局，结算期全部入口仍保持在可见区内。
- 上家、本家铭牌向左贴近安全边缘，下家铭牌向右贴近安全边缘。
- 四家铭牌使用半透明烟熏翡翠玻璃底，保留铜边、高光与文字描边。

## 2.6.50 - 2026-09-11

- 扩大四家铭牌和主要文字，左上工具改为大图标、向下展开的覆盖式抽屉，并将结算与下一局入口统一收纳其中。
- 定缺条、筒、万圆印和文字扩大一倍；定缺阶段仍可随时展开或缩进左上工具。
- 修正手牌展示优先级：默认胡牌仍盖牌，开启明牌及牌局结算时则翻开四家全部剩余暗手，副露保持原展示逻辑。

## 2.6.49 - 2026-09-10

- 将课程的“不退进听、再比活张”改写为普通进攻的分层排序，并保留防守、保分和追分分支。
- 加入四类经典牌型的互斥拆分与破平信号，修正多对子中额外对子应视作未完成搭的结构计数。
- 补入 20 道遮答案课程题的输入与独立答案键，并修正原图中 22-1 同时接受 2 条和 5 条。
- 修正明杠应计一根、金钩钓加根及自摸加底的计分投影；增加对应聚焦校验。

## 2.6.48 - 2026-08-22

- 重做六套桌布的碰、杠、胡、取消按钮面：使用各桌布实际 albedo 布料贴图生成独立圆章，保留单层老铜包边、材质阴影和大字，替代上一版统一平涂样式。
- 切换桌布时动作栏同步切换真实圆章纹理；不改麻将玩法、计分、触控或动作信号。

## 2.6.47 - 2026-08-22

- 重新设计碰、杠、胡、取消动作按钮：保留单圈识别，去除圈中圈复杂装饰，放大中文动作文字并加入暖象牙描边和轻阴影。
- 动作按钮绑定当前桌布皮肤的材质色板；切换六套桌布时同步刷新按钮颜色与高光，玩法和触控行为不变。

## 2.6.46 - 2026-08-22

- 碰、杠、胡操作按钮触摸区域和视觉尺寸扩大一倍，并同步调整动作栏布局。
- 补杠候选不再限制为“刚摸到的那一张牌”：碰牌后第四张牌留在手牌时，后续自己的摸牌回合仍显示补杠并按补杠分结算。
- 新增“保留第四张牌后补杠”的规则与计分回归测试。

## 2.6.45 - 2026-08-21

- 修正碰/杠/胡来源箭头挂点：改为牌面中心，不再偏到牌外。
- 将来源箭头从零厚度平面改为带实体厚度的 3D 箭头，按来源座位方向旋转。

## 2.6.44 - 2026-08-21

- 将碰/杠/胡来源箭头改为与中心弃牌黄色菱形一致的金黄色高质感材质。
- 放大箭头头部与箭杆，并保留来源座位方向标注；不改变碰、杠、胡及牌局逻辑。

## 2.6.43 - 2026-08-21

- 将四川牌桌中心区域迁移为内江最近版本的单环深玉中心仪表：连续深玉底座、四块齐平活动色嵌片、珠白分隔线、单个金色圆环和数字牌盘。
- 取消旧四川中心的玻璃菱形、红色活动区域和东南西北方向文字；保留四家轮转状态、余牌数字和静态移动端成本。
- 仅替换中心区域表现与对应 3D 资源，不改变四川麻将规则、AI、牌局流程、牌体和其他桌面区域。

## 2.6.42 - 2026-08-21

- 缩小青黛格绒的格纹单元：UV 密度提升至 6.0，降低法线强度；其他五套桌布保持不变。

## 2.6.41 - 2026-08-21

- 放大左上角缩进按钮的可视与触摸目标，避免 iPhone 上难以点击。
- 换肤弹窗先进入可见状态，再延后一帧整理选中态和焦点；移除入口处多余的工具栏重排，降低首次打开等待。
- 统一六套桌布的纹理密度、法线强度、粗糙度和翡翠/铜色配色，使纹理不再过大或过碎，并与现有牌桌风格协调。

## 2.6.40 - 2026-08-21

- 修复桌布皮肤入口点击后整个工具栏收回的问题：换肤面板打开时不再调用强制收缩，工具栏保持展开，面板仍在上层拦截自身触摸。
- 保留 2.6.39 的触摸卡片命中修复；本次仅调整界面层交互，不改变麻将玩法、AI、牌体或规则。

## 2.6.39 - 2026-08-21

- 修复 iOS 桌布皮肤按钮“能打开但卡片不能切换”的问题：模态换肤层明确处理触摸卡片命中，重复鼠标事件只做去重，不再吞掉皮肤选择。
- 修复入口反馈：打开桌布皮肤弹窗不再强制收回已展开的工具栏，避免手机上出现“点击后整个收缩框收回”的误导状态。
- 新增生产场景触摸选择回归，确认卡片选择会同步 `MainSceneV2`、`SichuanTableStage3D` 和本地皮肤偏好；不改变四川麻将玩法、AI 或牌桌几何。

## 2.6.38 - 2026-08-20

- 新增桌布换肤面板与六套 Poly Haven CC0 PBR 布料皮肤：深翡翠绉绒、翡翠精纺、墨绿暖绒、黑金暗纹、香槟缎面和青黛格绒；选择会保存到本地并在下次启动恢复。
- 换肤严格限定为 `TableFelt` 材质贴图与参数，不修改麻将牌、桌体几何、相机、触控、四川规则、计分或 AI；继续保持移动端 2 盏实时灯、1 盏投影灯的既有预算。
- 换肤面板适配安全区、触摸目标和键盘焦点，打开时拦截下层麻将输入；新增六套资源、生产入口、状态持久化和界面隔离的聚焦验证。
