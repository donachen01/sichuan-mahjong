...
`2.6.31` 修复 2.6.30 桌布底色多尺度调制经 iOS mipmap 过滤后呈现为大片云状水渍的问题：Blender 5.2 生成器改为均匀自然暖绿 BaseColor，细密短绒只保留在高频 Normal 和 `0.86–0.90` 粗糙度中。真实桌面顶面 UV 的低频能量占比、低频亮度跨度、法线低频比例和粗糙度跨度现由专项验证器约束；两次 Blender 生成哈希一致，两档 2556×1179 Metal 成品桌布中位色均为 `RGB(53, 131, 64)`。固定 27 项回归清单已按用户指示删除；单项 Runner 保留，只按每次修改的直接风险选择执行。桌体几何、UV、灯光、镜头、布局、触控、规则、计分和 AI 不变。

`2.6.30` 将牌局桌布重建为启动图同类的自然暖绿均衡短绒 PBR：固定种子的 Blender 生成器输出 2048 BaseColor、Normal、ORM，Normal 使用四组近等权、多方向高频绒毛场，移除旧版偏青底色和纵向织带；roughness 收敛到 `0.86–0.90`，metallic 为 `0`。限定暖光校准后的四档 Metal 成品桌布均为 `RGB(53, 131, 64)`，接近启动图基准约 `RGB(54, 130, 64)`。启动图仅用于视觉校准，未被采样或复制；木框、皮革、凹槽、中心方向器、镜头、布局、触控、规则、计分、AI 和牌面补光不变。

`2.6.29` 修正四个激活状态片被错误地统一为 `90°` 的几何：真实石墨分隔线对应东/西 `124.48292°`、南/北 `55.51708°`，状态片现与分隔线共用角度参数，东/西计数器两侧原本未填红的楔形完整覆盖，同时继续用半径 `0.460` 的切口避让 `0.455` 古铜表圈。结算页改为单层深翡翠主壳和无重复套框的内容分区；四家列表及明细行按可用高度扩展，主标题、玩家信息、统计、焦点分数和表格正文放大，展示字段、账本来源和触控切换保持不变。
...
`2.6.27` 将中心四向器重做为与深翡翠牌桌齐平的亮面玻璃嵌件：连续烟熏翡翠玻璃、共面石墨方向发丝线、干净且无黄色/旧铜描边的外缘、独立低高光古铜计数环和玻璃镜片各自使用 Blender PBR 材质，不再发生透明组件串色。当前方位使用真实 3D 顶面网格和不受桌面暖光/清漆高光漂移的状态材质，最终 Metal 像素锁定为用户指定的 `#A13D2D`（RGB `161, 61, 45`）；每个方向由主梯形和左右两个切角三角形组成，轮到东/南/西/北时会从计数圈完整填充至直边和切角，不留下旧边线或黑侧壁。模型仅 `988` 三角面、`9` 个静态对象和 `4` 个材质。桌布移除会读成水渍的大块低频颜色噪声，改为均匀森林绿底色加细密短绒法线与微粗糙度；八条分界线沉入绒布顶面并保持近色暗纹。三家立牌按共享 `0.42 × 0.24 × 0.58` GLB 的真实半高计算接触位置，牌底留 `0.012` 世界单位阴影缝，不再嵌入桌布。本家手牌继续使用已验证的 `0.83` 紧凑正间隙，Android 继续使用 `gl_compatibility` 与 `2048/质量 0` 移动阴影预算。

`2.6.25` 将本家手牌调整为 `0.83` 中心距，在保持原参考图紧凑节奏的同时，为 `0.8148` 世界宽度的正常牌体保留约 `0.0152` 实体缝隙，逐牌边界恢复清晰；中心四向盘改为暗色底座、蓝钢框、金属倒角、香槟色内嵌边、四条分区接缝和三级圆形表圈组成的低矮实体模型。中心组件全部静态、关闭投影且不增加逐帧动画，Android 继续使用 `gl_compatibility` 与 `2048/质量 0` 移动阴影预算。

`2.6.24` 按原版牌桌参考图把本家手牌恢复为固定 `0.80` 中心距，撤销 2.6.23 过大的正间隙，使 14 张牌紧密排齐并仅保留牌体边缘细缝；中心四向盘继续以红色梯形和亮色方位字共同标示当前回合。Android 保留 Godot 4.6 的 `2048/质量 0` 移动阴影预算与 `gl_compatibility`，继续修复相较 2.6.16 的卡顿退化；牌体大小、规则、AI、相机和触控合同不变。

`2.6.22` 在上述透视大小/厚度校准上继续修复其余三家胡牌、碰杠的白绿分层：共享 GLB 翡翠背层总内缩从 `0.018` 收紧为 `0.006`，平扣覆盖层与物理绿背统一为 `0.414×0.574`，只保留窄象牙唇边，白绿读成一块完整实体牌。上/下家平扣 `1.34`、对家平扣 `1.56`、本家碰/杠 `0.84×` 不变；同一 `0.42×0.24×0.58` GLB 继续三轴统一缩放，未改变规则、AI、相机或触控。Android 延续 `gl_compatibility`，iOS 延续 Forward+。
...
- 结算最终收分继续以权威总账为准；牌墙流局查叫只在尚未胡牌的玩家之间收付，已胡玩家不会再重复获得查叫分，所有有符号明细相加严格等于顶部和左侧最终收分。
