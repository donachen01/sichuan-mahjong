...
func _run() -> void:
	...
	if world != null and world.environment != null:
		...
		_check(environment.ambient_light_energy >= 0.20 and environment.ambient_light_energy <= 0.25, "ambient energy lifts mobile shadow detail without flattening the felt")
		...
		_check(environment.ssao_intensity >= 0.70 and environment.ssao_intensity <= 0.95, "SSAO intensity remains contact-local instead of forming black troughs")
	...
	if key != null:
		...
		_check(key.light_energy >= 0.76 and key.light_energy <= 0.82, "key energy preserves ivory detail without clipping tile tops")
		_check(key.shadow_opacity >= 0.58 and key.shadow_opacity <= 0.68, "key shadow opacity keeps racks grounded without black fence silhouettes")
		...
		_check(ray_direction.y < -0.84, "key light is overhead enough to keep wall shadows compact")
		_check(ray_direction.x > 0.02 and ray_direction.z > 0.10, "shadow travel keeps a restrained screen right/down direction")
	if fill != null:
		_check(not fill.shadow_enabled, "fill light never creates a second shadow")
		_check(fill.light_energy >= 0.90 and fill.light_energy <= 1.35, "tile-only fill preserves glyphs without bleaching ivory surfaces")
		_check(fill.light_color.r >= fill.light_color.b and fill.light_color.g >= fill.light_color.b * 0.94, "tile-only fill stays warm-neutral instead of turning ivory grey-blue")
