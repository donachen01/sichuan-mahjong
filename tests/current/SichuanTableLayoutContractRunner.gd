...
func _verify_live_scene(failures: Array[String]) -> void:
	...
	if seat_huds.size() != 4:
		...
		if not left_hud.size.is_equal_approx(right_hud.size):
			failures.append("left and right SeatHUD sizes must match")
		...
		if top_hud.get_global_rect().get_center().x <= root_ui.get_global_rect().get_center().x:
			failures.append("top SeatHUD must dock to the right side of the top tile track")
...
func _verify_center_and_discard_layer(scene: Node, failures: Array[String]) -> void:
	...
	if indicator == null or indicator.get_script() == null or indicator.get_script().resource_path != CENTER_INDICATOR_PATH:
		...
		indicator.call("render", 55, 3, "当前：下家")
		...
		if str(indicator.call("get_active_direction_text")) != "":
			failures.append("中心余牌区不得显示东西南北方位字")
