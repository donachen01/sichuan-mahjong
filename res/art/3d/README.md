...
| `sichuan_center_compass_v2.glb` | `tools/3d/generate_sichuan_center_compass_v2.py` | 0 | 1,044 三角面、9 对象、4 材质的齐平亮面玻璃四向嵌件；Blender 提供连续烟熏翡翠玻璃、石墨方向发丝线、无黄色/旧铜描边的干净外缘、低高光古铜计数环、玻璃镜片，以及使用半径 0.460 分段圆弧切口避让 0.455 古铜计数环、经 Filmic 显示补偿后输出 `#A13D2D` 的真实 3D 状态片；Godot 仅叠加动态余牌数字与方向字 |
